...
// 	};
// }
var arrEvent=['click', 'mouseover', 'mouseout', 'dblclick'];
for (var i = 0; i < arrEvent.length; i++) {
	addEv(arrEvent[i]);
};
function addEv(eventName){
	ZQuery.prototype[eventName]=function(fn){
		for (var i = 0; i < this.elements.length; i++) {
...
function domReady(fn){
	if(document.addEventListener){
		...
		document.attachEvent('onreadysatechange',function(){
			if(document.readyState=='complete' || document.readyState=='loaded'){
				fn && fn();
			}
		})
