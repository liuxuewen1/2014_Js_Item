 ...
 */
 ...
 //detail
 //为什么传window？1、不需要去外层找window，提升查找效率；2、由于window作为变量传进来，压缩时可以提高压缩率
 //为什么传undefined？1、undefined在IE8及以下可能被修改成其他的值，所以防止这种情况，做undefined初始化
...
var
	...
	//detail 存储jQuery元素的根元素，rootjQuery=jQuery(document);
	//为什么要定义成变量？提高压缩率
	rootjQuery,	

	// Support: IE<10
	// For `typeof xmlNode.method` instead of `xmlNode.method !== undefined`
	//detail 为什么用 typeof undefined 代替 undefined ？
	core_strundefined = typeof undefined,

	// Use the correct document accordingly with window argument (sandbox)
	//detail 为什么存储在变量？1、提升查找速率；2、提高代码压缩率
	location = window.location,
	document = window.document,
	docElem = document.documentElement,

	// Map over jQuery in case of overwrite
	//detail 防冲突 如果jQuery在外面被使用，则将外面的jQuery挂到 _jQuery 上
	_jQuery = window.jQuery,

	// Map over the $ in case of overwrite
	//detail 防冲突 如果$在外面被使用，则将外面的$挂到 _$ 上
	_$ = window.$,

	// [[Class]] -> type pairs
	//detail 类型判断$.type() 的时候使用，类似于 calss2type={'[Object, String]': 'string', '[Object Array]': 'array'}
	class2type = {},
	...
	//IE678不支持 字符串.trim()去空格 需要自行用正则去干掉空格
	core_trim = core_version.trim,

	// Define a local copy of jQuery
	//detail jQuery的设计方式：详见 框架/jQuery设计方式.html
